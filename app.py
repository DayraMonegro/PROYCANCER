...
from models.base import  engine # Updated import
...
def api_cancer_data():
    print("API LLEGAMOS")
    cancer_data = db_session.query(Cancer).limit(1000)
    ##print(cancer_data)
    ...
    for dato in cancer_data:
        datos.append({
            ...
            "year":dato.year,
            "genetic_risk":dato.genetic_risk,
            "air_pollution":dato.air_pollution,
            "alcohol_use":dato.alcohol_use,
            "smoking":dato.smoking,
            "obesity_level":dato.obesity_level,
            "cancer_type":dato.cancer_type,
            "cancer_stage":dato.cancer_stage,
            "treatment_cost_usd":dato.treatment_cost_usd,
            "survival_years":dato.survival_years,
            "target_severity_score":dato.target_severity_score
        })
    ##print(datos)
...
def actualizar_cancer_record(id):
    try:
        ...
        record.genetic_risk = data.get("genetic_risk")
        record.air_pollution = data.get("air_pollution")
        record.alcohol_use = data.get("alcohol_use")
        record.smoking = data.get("smoking")
        record.obesity_level = data.get("obesity_level")
        record.cancer_type = data.get("cancer_type")
        record.cancer_stage = data.get("cancer_stage")
        record.treatment_cost_usd = data.get("treatment_cost_usd")
        record.survival_years = data.get("survival_years")
        record.target_severity_score = data.get("target_severity_score")
        db_session.commit()
...
def cancer_by_type():
    try:
        ...
        data = [{"cancer_type": r.cancer_type, "total": r.total} for r in records]
        return jsonify(data)
    except Exception as e:
...
def cancer_by_region():
    try:
        ...
        data = [{"country_region": r.country_region, "total": r.total} for r in records]
        return jsonify(data)
    except Exception as e:
